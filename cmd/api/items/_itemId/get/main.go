...
func Handler(ctx context.Context, event events.APIGatewayV2HTTPRequest) (events.APIGatewayV2HTTPResponse, error) {
	...
	item, err := dao.Item.GetById(ctx, event.PathParameters["itemId"])
	if err != nil {
		log.Printf("Unable to get item from the database: %s", err.Error())
		return api.InternalServerErrorResponse("Unable to get item from the database")
	}
